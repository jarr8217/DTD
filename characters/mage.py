...
class Mage(Character):
    ...
    def __init__(self, name):
        super().__init__(
            name, health=MAGE_CONFIG['max_health'], attack_power=MAGE_CONFIG['attack_power'])
    ...
    def arcane_fortify(self):
        ...
        if not self.buffed:
            self.max_health += 40
            # Ensure attack power doesn't go to 0
            ...
            print(
                f'{self.name} uses Arcane Fortify! Health increased by 20 and attack power halved for 2 turns!')
    ...
    def end_turn(self):
        ...
        if self.buffed:
            ...
            if self.arcane_fortify_buff_duration <= 0:
                ...
                self.max_health -= 40
                self.attack_power = self.original_attack_power  # Restore to original value
                self.buffed = False
                # Make sure health does not exceed max_health
                if self.health > self.max_health:
                    self.health = self.max_health
                print(
                    f'{self.name}\'s Arcane Fortify buff has worn off! Health and attack power restored to normal.')
        ...
        if self.arcane_fortify_cooldown > 0:
            ...
            if self.arcane_fortify_cooldown == 0:
                ...
                print(
                    f'{self.name}\'s Arcane Fortify cooldown: {self.arcane_fortify_cooldown} turns remaining.')
