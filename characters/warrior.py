...
class Warrior(Character):
    ...
    def __init__(self, name):
        super().__init__(name,
                         health=WARRIOR_CONFIG['max_health'], attack_power=WARRIOR_CONFIG['attack_power'])
    ...
    def holy_light(self):
        if self.holy_light_cooldown > 0:
            print(
                f'{self.name}\' is on cooldown for {self.holy_light_cooldown} more turns!')
            return
        ...
        print(
            f'{self.name} uses Holy Light! Restores {heal_amount} health and gains +15 attack power!')
    ...
    def end_turn(self):
        ...
        if self.holy_light_buff_active and self.holy_light_cooldown == 0:
            ...
            self.holy_light_buff_active = False
            print(
                f'{self.name}\'s Holy Light buff has worn off! Attack power restored to normal.')
        else:
            print(
                f'{self.name}\'s Holy Light buff is still active for {self.holy_light_cooldown} more turns!')
